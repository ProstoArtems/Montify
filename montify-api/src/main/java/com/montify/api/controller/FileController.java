...
public class FileController {
    ...
    public ResponseEntity<InputStreamResource> downloadUploadedFile(
            @PathVariable String sessionId,
            @PathVariable String fileKey,
            @RequestHeader(value = HttpHeaders.RANGE, required = false) String rangeHeader) {

        // 1. Вызываем сервис, передавая rangeHeader
        ...
        // 2. Если MinIO вернул часть контента (статус 206), обрабатываем его
        ...
        return responseBuilder
                .contentType(MediaType.parseMediaType("video/mp4")) // Убедитесь, что тип верный
    ...
    public ResponseEntity<InputStreamResource> exportVideo(
        ...
        // Если MinIO вернул часть контента (отвечая на наш Range запрос), возвращаем
        // 206 статус
        if (s3Response.contentRange() != null) {
            responseBuilder = ResponseEntity.status(HttpStatus.PARTIAL_CONTENT)
                    .header(HttpHeaders.CONTENT_RANGE, s3Response.contentRange());
        ...
        return responseBuilder
                ...
                .header(HttpHeaders.ACCEPT_RANGES, "bytes") // КРИТИЧНО: говорим браузеру, что поддерживаем перемотку по
                                                            // байтам
                .contentLength(s3Response.contentLength()) // Длина именно ТЕКУЩЕГО куска (чрезвычайно важно для 206)
                // Рекомендуется сменить attachment на inline, чтобы браузер воспринимал это как
                // потоковое видео, а не скачиваемый файл
